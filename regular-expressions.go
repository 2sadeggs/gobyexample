...
func main() {
	...
	//true
	fmt.Println(r.MatchString("pearch"))
	//true  也是true 说明匹配的不只是2个字符  再换一个单字符试试
	fmt.Println(r.MatchString("pzch"))
	// 也是true

	//查找匹配的字符串。
	fmt.Println(r.FindString("peach punch"))
	//peach 匹配到了
	fmt.Println(r.FindString("pech pach"))
	//输出pech
	fmt.Println(r.FindString("pach pech"))
	//输出pach 可见匹配到就不再往下找了

	//这个也是查找首次匹配的字符串， 但是它的返回值是，匹配开始和结束位置的索引，而不是匹配的内容。
	fmt.Println("idx: ", r.FindStringIndex("peach punck"))
	//idx:  [0 5]
	fmt.Println("idx: ", r.FindStringIndex("peach"))
	//idx:  [0 5]
	fmt.Println("idx: ", r.FindStringIndex("pearch"))
	//idx:  [0 6]
	...
	//[0 5 1 3]
	fmt.Println(r.FindStringSubmatchIndex("pearch"))
	//[0 6 1 4]
	fmt.Println(r.FindStringSubmatchIndex("pech"))
	//由此推断出 第二部分的两个inx值 是命中正则表达式值的前后索引
	...
	//[[0 5] [6 11] [12 17]]
	fmt.Println(r.FindAllStringIndex("peachpunchpinch", -1))
	//[[0 15]]
	...
	fmt.Println("regexp must compile: ", r)
	//regexp must compile:  p([a-z]+)ch
