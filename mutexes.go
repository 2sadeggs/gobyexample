...
func main() {
	...
	doIncrement := func(name string, n int) {
		for i := 0; i < n; i++ {
			c.inc(name)
			//wg.Done() //不是很理解这里wg.Done()的作用
		}
		wg.Done() //挪到这里就好懂了 加了三个锁 开启了三个协程 因此每个协程结束后wg -1
	}
	/*同时运行多个 goroutines; 请注意，它们都访问相同的 Container，其中两个访问相同的计数器。*/
	/*同时运行多个 goroutines; 请注意，它们都访问相同的 Container，其中两个访问相同的计数器。*/
	wg.Add(3)
	go doIncrement("a", 10000) //期望值不好说 因为两个协程都在自增 我觉得是10000 因为上了锁 另一个协程影响不到
	go doIncrement("a", 10000) //实际结果是20000 符合预期 因为执行了两遍 两遍各自运行 结果叠加
	go doIncrement("b", 10000) //期望值 10000
