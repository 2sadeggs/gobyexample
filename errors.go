...
/*
还可以通过实现 Error() 方法来自定义 error 类型。 这里使用自定义错误类型来表示上面例子中的参数错误。
Error() 方法的返回值是个string
*/
...
func main() {
	...
	*/
	...
	//注意是错误类型的数据  不是简单判断错误是否为nil 是要用错误数据 得先搞清楚错误类型
	//e.(*argError) 为断言语法 判断e是佛是*argError 类型
	_, e := f2(42)
