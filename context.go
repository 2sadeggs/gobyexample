...
context.Context 被用于控制 cancel。 C
ontext 跨 API 边界和协程携带了：deadline、取消信号以及其他请求范围的值。*/
...
func hello(w http.ResponseWriter, r *http.Request) {
	...
	case <-time.After(10 * time.Second):
		fmt.Fprintf(w, "hello\n")
	case <-ctx.Done():
		err := ctx.Err() //context 的 Err() 方法返回一个错误， 该错误说明了 Done 通道关闭的原因。
...
/*PS D:\GolandProjects\gobyexample> go run .\context.go
